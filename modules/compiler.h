  ...
  string asm_get_value(string &value, Procedure &proc){
    if(lex::literal_or_var(value) == lex::ERROR){
      ...
      switch(size){
        ...
        break;
        default:
        cerr << "\033[1;31mUknown size of '"<<size<<"' when getting the value of '"<<value<<"'.\033[0m\n";
        assert(false);
        break;
      }
  ...
  class COMPILER{
    ...
    void preparse(){
      ...
      for(int i=0; i<INTER.size(); i++){
        ...
        else if(cur == "def"&&(INTER[i-1]!="static"||INTER[i-1]!="extern")){
      ...
      for(int i=0; i<INTER.size(); i++){
        ...
        if(cur == "proc"){
          ...
          if(INTER[i+1]=="="){
            ...
            Variable var = PROCS[CP].VARS[sym];
            switch(size){
              case 1:  DATA_SECTION += "  "+var.label+": db 0\n";                           break;
              case 2:  DATA_SECTION += "  "+var.label+": dw 0\n";                           break;
              case 4:  DATA_SECTION += "  "+var.label+": dd 0\n";                           break;
              case 8:  DATA_SECTION += "  "+var.label+": dq 0\n";                           break;
              default: DATA_SECTION += "  "+var.label+": times "+to_string(size)+" db 0\n"; break;
            }
          }
          i++;
        }
